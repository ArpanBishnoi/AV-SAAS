...
class USERINPUT(BaseModel):
    ...
    email: str = Field(min_length=5,max_length=60)
...
def update_notes(item: UPDATENOTEINPUT,user_id : int = Depends(get_current_user)):
    cursor.execute("UPDATE Notes SET content = ? WHERE id = ? AND user_id = ?", (item.content, item.id,user_id))
    conn.commit()
...
def delete_notes(delete_id: int,user_id : int = Depends(get_current_user)):
    cursor.execute(
        "Delete FROM Notes WHERE id =? and user_id = ?", (delete_id,user_id)
